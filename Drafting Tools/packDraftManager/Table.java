...
public class Table {
	...
	public static void main(String[] args) {
		...
		fillPools();
		print("How many players in this draft?");
	...
	private static void printEachPlayersArsenal() {
		for (Player p : players) {
			print(p.getName() + ": " + p.getHandAsString());
		}
		print("");
	}

	private static void askPlayerToPickOne(Player p) {
		print(p.getName() + ", your picks are as follows!" + '\n' + "(Already in your arsenal: "
				+ p.getDeckAsString() + ")");
		p.printPicks();
	...
	private static void createPlayers(int playerNo) {
		for (int i = 1; i <= playerNo; i++) {
			print("Gimme the name of player " + i + ".");
			waitForUserSelection();
			players.add(new Player(gui.userSelection));
			gui.userSelection = null;
		}
	}

	private static void waitForUserSelection() {
		while (gui.userSelection == null) {
			gui.userSelection = null;
		}
	}
